...
function getGlyph(symbol, font, pixelRatio) {
  ...
  if (symbol === '◇') { // placeholder for the fastest symbol in the west
    triSymbol.cells = [[0,1,2]]
    const hs = .35
    triSymbol.positions = [[-hs, -0.288*2*hs], [hs, -0.288*2*hs], [0, 0.288*2*2*hs]]
    lineSymbol = { edges:[], positions:[] }
  }
  //Save cached symbol
